...
{#if $showSidebar}
	<div
		class=" {$isApp
			? ' ml-[4.5rem] md:ml-0'
			: ''} absolute md:hidden z-40 inset-0 bg-black/60 w-full h-full flex justify-center overflow-hidden overscroll-contain"
		on:mousedown={() => {
							...
							{#if $chats}
								{#each $chats as chat, idx (`chat-${chat?.id ?? idx}`)}
									{#if idx === 0 || (idx > 0 && chat.time_range !== $chats[idx - 1].time_range)}
										<div
											class="w-full pr-2.5 text-right text-xs font-medium {idx ===
											0
												? ''
												: 'pt-5'} pb-1.5"
											style="color: #26849c;"
